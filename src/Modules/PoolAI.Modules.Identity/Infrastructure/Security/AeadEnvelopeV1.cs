#pragma warning disable MA0048 // Envelope document and key-ring types are one private implementation unit.
...
internal sealed partial class AeadEnvelopeV1
{
    ...
    internal JsonElement Encrypt(ReadOnlySpan<byte> plaintext, string aadText)
    {
        ...
        {
            throw new ArgumentException("Envelope plaintext is required.", nameof(plaintext));
        }
    ...
    private static ParsedEnvelope Parse(JsonElement envelope)
    {
        ...
        try
        {
            wrappedDek = DecodeLength(envelope, "wrapped_dek", KeySize);
    ...
    private static void ValidateAad(string aadText)
    {
        if (string.IsNullOrWhiteSpace(aadText))
        {
            throw new ArgumentException("Envelope AAD is required.", nameof(aadText));
        }
    }
