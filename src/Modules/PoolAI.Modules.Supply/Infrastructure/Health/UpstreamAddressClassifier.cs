...
internal static class UpstreamAddressClassifier
{
    internal static bool IsAllowed(
        ...
        IReadOnlyList<AccountHealthProbeHttpOptions.UpstreamPrivateEgressRule>?
            privateEgressRules = null)
    {
        ArgumentNullException.ThrowIfNull(requestUri);
        ArgumentNullException.ThrowIfNull(address);
        IPAddress canonical =
            AccountHealthProbeHttpOptions.UpstreamPrivateEgressRule
        ...
        bool loopbackHttp =
            ...
            && IsExactLoopbackHost(NormalizeHost(requestUri));
        if (loopbackHttp && allowLoopbackHttp)
        {
        ...
        if (IsAlwaysForbidden(canonical))
        {
            return false;
        }

        if (AccountHealthProbeHttpOptions.UpstreamPrivateEgressRule
            .IsPrivateAddress(canonical))
        {
            return privateEgressRules?.Any(rule =>
                rule.Matches(requestUri, canonical)) == true;
        }

        return !IsReservedIpv4(canonical)
            && !IsReservedIpv6(canonical);
    }
