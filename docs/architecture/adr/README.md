...
Proposed decisions (not in effect):

- [`0009-introduce-shared-secrets-envelope-runtime.md`](0009-introduce-shared-secrets-envelope-runtime.md) — propose a BCL-only `PoolAI.Infrastructure.Secrets` runtime shared only by Identity/Supply Infrastructure. Its Issue #44 approval evidence is pending; candidate code may be reviewed, but merge, release, and key operations are not authorized.
