...
| GroupQuota | Group 生命周期、runtime admission policy、activation evidence、Quota Ledger、Period、Reservation、核销与纠错 | Group、exact `runtime_policy`、opaque Supply readiness evidence、quota/period/reservation/event、`usage_requests`、`usage_attempts`、`usage_attempt_adjustments` 及 settlement event 语义 | Group/Quota Snapshot、Ledger 命令端口、版本化 settlement Integration Event |
...
| Routing | 同 Group 过滤、评分、粘性、Account lease、Account breaker/probe 与容量协调 | Redis 中的 lease、RPM、粘性、breaker/cooldown/probe 和短期协调状态；不拥有累计 Token 或持久 Account health 事实 | Router、Lease、Breaker 与候选决策端口 |
...
| Usage | Group/Account 查询、小时聚合、checkpoint-aligned 对账与读模型新鲜度 | `group_usage_hourly`、`account_usage_hourly`、`aggregation_watermarks` 等可重建投影；不拥有 GroupQuota 事实或通用 Inbox 表 | Pool Usage、Account Usage 与 quota reconciliation Query DTO |
...
- 已接受的 [`ADR 0009`](adr/0009-introduce-shared-secrets-envelope-runtime.md) 由 `@Lyon1984` 的 [Issue #44 永久架构批准](https://github.com/Lyon1984/PoolAI/issues/44#issuecomment-5117402125) 支持：`PoolAI.Infrastructure.Secrets` 是无业务所有权、只依赖 BCL 的共享技术 Infrastructure，仅承载严格 Envelope v1 解析/序列化、AEAD、精确 `kid` keyring 与 authenticated rewrap mechanics；它不拥有配置加载、日志/告警、业务 AAD、存储/CAS、轮换编排、备份恢复或 runbook。该架构批准不替代 migration 0010 的独立数据库签核；0010 随后由 [Issue #44 独立数据库评论](https://github.com/Lyon1984/PoolAI/issues/44#issuecomment-5117955336) 精确冻结候选 `6278efb09a308a011fc9c7c6175a269fba9faedd`、SQL SHA-256 `0de4b5e38ab8dee9f7d5b222e729947e825934241f2c9b8ae2598f89267174c0` 与 manifest `10..10`。两条批准均不授权远程 migration、Worker/rewrap/key operation、PR Ready/merge、部署或 M2-E1/发布验收。
- 已接受的 [`ADR 0011`](adr/0011-freeze-upstream-health-and-breaker-boundary.md) 通过 `@Lyon1984` 的 [Issue #44 永久架构批准](https://github.com/Lyon1984/PoolAI/issues/44#issuecomment-5135660266) 把 Worker 的 Routing 加载面限制为独立 health-only graph：Routing 继续拥有共享 breaker/probe，Supply 继续拥有 credential、主动探测 adapter 与持久 health，Operations 只提供 manifest-bound Redis/session-lock mechanics。它同时冻结 ADR 0010 留待首个 outbound milestone 完成的连接期 SSRF 边界；精确 Redis ABI 另由 [独立永久评论](https://github.com/Lyon1984/PoolAI/issues/44#issuecomment-5135961558) 批准，二者均不授权远程执行、部署或 M2-E4/M2 Exit。
- Accepted [`ADR 0015`](adr/0015-freeze-m4-e1-gateway-admission-and-upstream-handoff.md) 已由 `@Lyon1984` 的 [Issue #44 永久架构批准](https://github.com/Lyon1984/PoolAI/issues/44#issuecomment-5506352845)，登记 M4-E1 的完整 per-Group RPM authority、trusted-proxy client-IP resolution、`conservative-v1` estimate、既有 lossless usage 纠正和 revision-fenced route/credential handoff。精确 [OpenAPI compatibility-window](https://github.com/Lyon1984/PoolAI/issues/44#issuecomment-5506358261) 与 [migration 0019](https://github.com/Lyon1984/PoolAI/issues/44#issuecomment-5506361132) 已分别独立批准，且均不授权远程数据库、Redis、upstream 或 credential 操作。
...
| GroupQuota | SubscriptionAccess、Gateway、Usage | 同步 Group/Quota 端口；Gateway 强读 Group-owned runtime admission policy；结算事实通过版本化 outbox 事件异步发布 | Usage 修改 quota，或查询方以聚合结果/Redis/配置决定准入或 Group RPM limit |
| Supply | Routing、Gateway、Group Activation Orchestrator | 同步 Supply Configuration/Readiness/候选端口；候选携带非秘密 route/version/`credential_revision`，并向 Gateway 暴露 revision-fenced one-use credential lease、向 Routing 暴露 active probe 与持久 health writer 端口 | 共享 Account credential Entity/明文/envelope、由调用方写 `group_supply_configurations`/`group_accounts`/Account health，或 Supply 反向引用 Routing |
| Routing | Gateway | 同步策略/lease/breaker observation 端口；返回有生命周期的 lease/probe handle | Gateway 直接写 Redis key、复制 Lua、修改持久 Account health，或让 breaker 自行决定重放 |
...
3. **Group–Subscription lifecycle fence**。`poolai_group_update` 与精确新 identity `poolai_group_update_v2` 仅在 archive 分支、持有 quota → Group fence 后读取 `subscriptions(group_id,status,expires_at)`；`poolai_subscription_template_create/update/retire` 与 `poolai_subscription_assign/update` 仅可读取/锁定 `groups(id,status)`，并按 `Quota → Group → Template/Subscription` 全局偏序取得所需行锁。等待后的时间判定重新读取 PostgreSQL `clock_timestamp()`。`poolai_group_create_v2` 不加入跨 Context allowlist；v2 update 不扩大表、字段、锁方向或写权限。

三个 family 都绝不授权跨 Context INSERT/UPDATE/DELETE/MERGE、额外 function/table/field/lock direction、dynamic/general SQL executor、共享 DbContext/Repository 或复用到其他用例。ADR 0006 已 Accepted；Accepted ADR 0015 与已独立批准 migration 0019 共同使上述精确 v2 update identity 成为有效 allowlist，且不扩大其他函数、表、字段或锁方向。Architecture/Integration Test 必须证明完整函数/表/字段白名单、只读边界、锁序、等待后 DB clock、两种提交顺序和直接跨表写入被拒绝；除此之外不允许任何跨 Context 事务或表直读例外。
...
| Module Infrastructure | 本模块 Application Ports 与 Domain、必要的 EF/Npgsql/Redis/SMTP/加密实现；可引用 `PoolAI.Infrastructure.Postgres`；依据 Accepted ADR 0009，仅 Identity/Supply Infrastructure 可引用 `PoolAI.Infrastructure.Secrets` | Endpoints、其他模块实现或其 DbContext/Entity；不得把共享 PostgreSQL runtime 当业务 SQL/Repository hub，也不得把 Secrets runtime 当配置/日志/业务 AAD/存储 hub |
...
| `PoolAI.Infrastructure.Secrets`（Accepted ADR 0009） | 仅 BCL；仓库统一 `PrivateAssets=all` analyzer 只能提供 build-time 规则且不得进入 compile/runtime/native graph | 所有项目/framework/compile/runtime NuGet 引用、项目专属 source generator、业务模块类型、BuildingBlocks、Contracts、配置/日志/业务 AAD/存储/CAS/轮换编排、ASP.NET/EF/Npgsql/Redis/外部 SDK |
...
- 依据 Accepted ADR 0009，Identity/Supply Infrastructure adapter 可以调用 BCL-only `PoolAI.Infrastructure.Secrets` 的 generic Envelope v1 mechanics；模块负责从已验证配置构造 immutable keyring、定义并校验业务 purpose/entity/field AAD、持久化/CAS 与安全告警。共享 core 不读取配置、不记录日志、不决定业务 AAD、不访问存储，也不编排轮换/恢复。Account credential 的 selector、create/replace 与 maintenance CAS 只属于 Supply：非公开 `credential_revision` 与公开 Account `version` 分离，维护 rewrap 只推进前者并保持 content ciphertext/nonce/tag 不变；Operations 只提供通用 session advisory lock 与运维事件，不能接收 Envelope 或拥有 Account SQL。
- 外部协议变化只进入 Adapter 和 Gateway 规范化映射；Domain/Application 不使用 OpenAI DTO 作为自己的模型。Gateway 只把 Supply 的 revision-fenced credential lease 包装为 `Gateway.Abstractions` 定义的 opaque、one-use、zeroizing handle；Adapter 仍只依赖 `Gateway.Abstractions`，不得引用 Supply 实现或取得 secret bytes/string/envelope。
...
| Worker | `PoolAI.Worker/Program.cs` | `PoolAI.Infrastructure.Postgres` runtime/session lock、email/outbox、reservation、Supply health；依据 Accepted ADR 0011 仅含 breaker/probe + health coordinator 的 Routing health-only graph；Usage projection、独立 quota reconciliation 扫描/指标/告警等 Worker 能力 | 完整 Routing module/普通 Router/affinity/Group RPM、Gateway、公开 Endpoint、协议 Adapter、Api ServiceProvider、DDL runner、直接引用/注册 `PoolAI.Infrastructure.Secrets` |
...
模块通过显式 `AddIdentityModule()`、`AddGroupQuotaModule()` 等扩展注册。Api/Worker 在各自 Host 根部显式注册独立的 PostgreSQL data source/UoW runtime；不得跨进程共享 data source、connection、UoW 或根 `IServiceProvider`。依据 Accepted ADR 0009，Identity/Supply 的模块注册边界可把 Host 已验证的 secret-provider 配置转换为模块 Infrastructure 使用的 immutable keyring，但 Api/Worker 项目本身不得直接引用、注册或解析 `PoolAI.Infrastructure.Secrets`。通用模块注册扩展只做 binding、Options 校验和健康检查注册，不解析服务、不执行数据库写入、不启动后台循环，也不得调用 `BuildServiceProvider()`；只允许 Worker Composition Root 另行显式调用默认禁用的 Supply credential rewrap Worker 注册，Api 不得调用或解析该 HostedService。Accepted ADR 0011 登记一个精确例外：Worker 可调用独立 `AddRoutingHealthModule(IConfiguration)` 关闭 breaker/probe + Supply Health coordinator 对象图，但不得调用完整 `AddRoutingModule()`，不得解析普通 Router、affinity、Group RPM、Gateway、OpenAI Adapter 或 Endpoint；Api 的完整 Routing 注册不得启动该 Worker loop。对象图只能在对应 Host 根部闭合；测试可以建立测试专用 Composition Root，但生产模块不得感知它。Migrator 仍只有 migration-specific 数据路径，不以共享 runtime application UoW 取得 Schema 所有权，也不得加载 Secrets runtime。
...
投递语义为 at-least-once，不承诺全局顺序。需要顺序的消费者只能按已声明的 topic/aggregate sequence 处理；inbox receipt、投影写入和 checkpoint 前进必须在消费者的同一 UoW。重复消息结果不变，同 sequence 不同 payload hash 必须告警并停止该分区；未知 `schema_version` 不得静默丢弃。按 Accepted [`ADR 0012`](adr/0012-freeze-quota-event-replay-ordering-and-projection-boundary.md) 及其 [永久批准](https://github.com/Lyon1984/PoolAI/issues/44#issuecomment-5159233350)，`poolai.quota.v1` 的分区固定为 Group，逻辑 sequence 是 replay 保留的 `source_event_sequence`；物理 Outbox `event_sequence` 只承担 delivery/Inbox identity。该批准只冻结精确候选和 event schema，不单独构成 M3-E4 closeout 或发布证据。

GroupQuota Published Language 固定为 [`../contracts/group-quota-events-v1.json`](../contracts/group-quota-events-v1.json) 定义的 `topic=poolai.quota.v1`、`schema_version=1` `GroupQuotaEventV1` union，`event_type` 只能是 `initialized/reserved/dispatch_started/renewed/settled/released/expired/usage_adjusted/total_adjusted/period_reset`。Usage 仅对 `settled`、`expired` 且 `payload.metadata.conservative_expiry=true`、`usage_adjusted` 通过 GroupQuota Abstractions 的 immutable fact reader 重算完成小时；其他事件不增加 usage。不另发 `AttemptUsageRecordedV1/AttemptUsageAdjustedV1` 重复消息。GroupQuota 事件必须携带构建 Usage 投影所需的稳定字段或权威 fact reference，不允许 Usage 解析 GroupQuota 内部 Entity、获取其 Repository/DbContext 或直接查询事实表。新增必填字段或改变含义必须发布新 schema version；旧消费者的兼容/退出窗口由 ADR 和 contract test 固定。

按 Accepted ADR 0012，通用 Publisher 只 claim 已显式登记 consumer route 的 topic；对同一 quota Group，只允许最小未完成 `source_event_sequence` lineage 前进。dead lineage 只阻断该 Group，保留原 source sequence 的 pending replay 可从原逻辑位置恢复，其他 Group 可继续。Publisher 在 claim 事务提交后调用 durable consumer；只有 strict codec、fact-reference 校验和 consumer 单 UoW 成功或精确 Inbox duplicate 后，才以独立 owner/generation CAS 标为 `published`。未知 major、Envelope/payload/fact 不一致使 consumer 回滚、物理消息进入 dead并发 P0；发布确认前崩溃允许重复投递。该 publication guard 不把 consumer 事务并入 claim UoW，也不把已提交 producer 业务事务回滚。

依据 [`ADR 0013`](adr/0013-freeze-quota-reconciliation-layers-checkpoint-alignment-alerting-and-recovery-boundary.md)，quota reconciliation application/use case/Endpoint 由 Usage 拥有。Usage 只能通过 `GroupQuota.Abstractions` 的窄 fact reader 读取 selected period 当前账本、effective attempt/adjustment、pending reservation 与 exact logical checkpoint 的权威 consumed；Outbox/Inbox/backlog 只能通过 Operations 的窄诊断端口读取。任何模块都不得跨 context 直接取得另一 owner 的 Repository、DbContext、SQL 或写权限。API 通用 `AddUsageModule()` 不启动循环；只有 Worker Composition Root 显式注册独立 reconciliation service 与 session advisory lock。

公开 reconciliation 顶层字段继续只表达权威账实差异，既有 `is_reconciled` 真值表不变；可选 Usage projection 必须把 `aggregation_watermarks.last_event_sequence` 固定为 GroupQuota logical source checkpoint，并取该 sequence 之前 selected period 最后一个 `consumed_tokens_after` 比较投影。checkpoint 后的新事件只能形成 `lagging`，不得产生伪 `mismatched`。权威不一致时投影状态 `blocked`；detector 只读且不能静默改账，隔离/重启 lifecycle 必须经过既有 Admin command 与审计。
...
| Circuit Breaker | Routing 的 Account 级 closed/open/half-open 状态与跨 Api/Worker probe lease | Redis 共享窗口/单 probe，持久 health 取更严格状态；只记录可归责 outcome；新 Account unknown 走 Supply 受控主动验证而不制造 half-open | 每实例独立 breaker、把新 unknown 当 half-open、401/403 随时间自动探测、breaker 自行重放请求 |
...
Decorator 的固定外层顺序为：request correlation/observability → admission bulkhead → authentication → request validation/body limit → Gateway Process Manager。M4-E1 的模型调用在 canonical Key/User/Subscription/Group 强读后，从 GroupQuota-owned `runtime_policy` 取得 `requests_per_minute`，对入站 request 调用一次既有 Group RPM primitive，再做 map/estimate 与当前 Supply read；每个 attempt 随后固定为 route/Account lease → quota reserve → revision-fenced credential acquire/Adapter prepare → dispatch fence → upstream call/stream → settle/drain → credential/Account lease release。Models/Usage 与 failover 不重复计 RPM。只有 Process Manager 可以根据 capability 和 phase创建下一个 attempt；timeout、breaker 和 Adapter 都只能返回强类型 outcome，不能自行循环；实际 failover loop 属于 M4-E5，不得塞入 M4-E1。

Accepted ADR 0015 把 Group runtime policy 限制为精确 JSON `{"schema_version":1,"requests_per_minute":N}`，其中 `N` 为 `1..1,000,000`、create omission 固定为 `6000`、update omission 保持不变。任何 PATCH 提交 `requests_per_minute` 都必须同时提交 reason，并保持 Group command 的 `If-Match`、Idempotency-Key、单 UoW/单 version、audit/outbox 语义。Redis、全局配置、Supply、Subscription 和 activation evidence 都不能成为 limit authority；既有 Redis Group RPM key/Lua/TTL/`TIME` ABI 不改变。

可信代理解析只信任 socket `RemoteIpAddress` 起点与启动时验证的 `Gateway:Ingress:TrustedProxyCidrs`（默认空、最多 64 个 canonical CIDR，拒绝 `/0`/host bits/zone/mapped-v6/duplicate）和 `Gateway:Ingress:ForwardedForLimit`（默认 1，范围 1..8）。直接 peer 不可信时忽略所有 forwarding headers；可信时只允许一个、最多 1024 UTF-8 bytes 的 `X-Forwarded-For`，bare IP tokens 从右向左按 trusted chain 剥离，拒绝 malformed/port/quote/zone/unknown/obfuscated/ambiguous IPv4，绝不读取 `Forwarded`/`X-Real-IP`。API Key `allowed_cidrs=[]` 仍表示 unrestricted；否则 resolved client 必须匹配。malformed/mismatch 统一为既有 `invalid_api_key`，raw IP/header 不进入日志/trace/metric。

默认 Token estimate strategy 固定为 pure `conservative-v1`：以固定 compact `System.Text.Json` 设置序列化 normalized payload，`estimated_input_tokens=max(1, UTF8 byte length + 64)`；output 取显式 `max_output_tokens`/`max_completion_tokens`，否则取 `Gateway:DefaultMaxOutputTokens=4096`；checked `BigInteger` 相加并受 `Gateway:MaxEstimatedTokensPerAttempt=2,000,000` 约束，且启动要求 default 不大于 max。它不访问数据库/Redis/HTTP/provider SDK。超限在 route/lease/reserve 前返回既有 `422 validation_failed`，output-caused 指向具体 max 字段，input-only 指向 `/input` 或 `/messages`。

Adapter normalization 必须使用 `BigInteger` 的完整 `NormalizedUpstreamUsage`（input/output/cache-read/cache-creation/thinking + bounded raw evidence），不得把 upstream JSON 先转 `long/double/decimal`。事实先无损 settle；公开单请求 usage 超过 `2^53-1` 才按既有 `upstream_usage_out_of_range` 终止，超过数据库 78 位则整个事务回滚并走 `token_numeric_overflow` P0。Selected route 只带 IDs、provider、client/upstream model、canonical base URI、capabilities、Supply/Channel/Account versions 与 `credential_revision`；credential 只能经 Supply revision-fenced single-use lease 包成 Gateway opaque handle，并且只在 ADR 0010/0011 连接期 DNS/egress/Host/SNI/certificate fence 通过后附加 Authorization。

Account lease 执行中每 20 秒 renew；typed `Lost` 立即取消上游，连续两次 coordination failure 也取消。它与 reservation lifetime coordinator 汇入同一 attempt cancellation，随后最多 15 秒 bounded drain，再严格按 dispatch phase settle 或 release，finally 零化 credential handle 并释放 Account lease。renew/cancel/drain/settle/release 的等待都不得跨 PostgreSQL transaction。
...
7. Api/Worker/Migrator 彼此引用可执行项目，或加载超出各自 Composition Root 白名单的模块/loop/Adapter；尤其是 Api/Worker 未各自注册 Host-local PostgreSQL runtime、任一 Host 直接引用/注册 `PoolAI.Infrastructure.Secrets`，或 Migrator 注册 runtime application UoW/业务模块/Secrets runtime。依据 Accepted ADR 0011，Architecture Test 还必须证明 Worker 只调用一次 Routing health-only 注册且没有完整 Router/affinity/Group RPM/Gateway/Adapter/Endpoint，Api 不注册 Supply Health loop。
