...
| Supply | Routing、Gateway、Group Activation Orchestrator | 同步 Supply Configuration/Readiness/候选端口；向 Routing 暴露受控 credential lease、active probe 与持久 health writer 端口 | 共享 Account credential Entity、由调用方写 `group_supply_configurations`/`group_accounts`/Account health，或 Supply 反向引用 Routing |
