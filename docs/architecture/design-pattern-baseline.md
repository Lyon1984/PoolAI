...
R1 只登记两个跨 Context 数据只读窄例外。第一，为保证撤销与 Group quota 准入具有单一线性化顺序，GroupQuota Infrastructure 的 `poolai_quota_reserve` Transaction Script 可以在自身短事务内以固定顺序只读并锁定 `users/user_roles/api_keys`、`subscriptions`、`groups`、`group_supply_configurations/group_accounts/accounts/channels` 的 canonical 行，验证请求 Channel 等于当前配置 Channel，再写 GroupQuota 所有的 request/reservation/quota/event/settlement 与当前 UoW 的 Operations append 表。该脚本只能读取 ID、归属、status/version、有效期、模型绑定、health/cooldown 等已在数据库契约列明的 `QuotaAdmissionSnapshot` 字段；不得读取 credential/password/secret，不得 UPDATE 其他 Context 的业务列，不得扩展为通用跨模块 DbContext/Repository，也不得被非 quota 用例复用。第二，`poolai_validate_group_activation` 数据库 guard 可以在 Group 状态变为 active 的语句时只读 quota、`group_supply_configurations` 与 Supply readiness 字段，防止绕过 Orchestrator 的明显非法写入；它不锁定或写 Supply，不把点时 readiness 提升为跨 Context 永久不变量，正常入口仍必须经过 `GroupActivationOrchestrator` 并记录 opaque readiness evidence。锁序、列级权限和所有等待后的数据库时钟重检以数据库契约为准，Architecture/Integration Test 必须证明固定表/字段白名单、只读边界、revoke/reserve 两种提交顺序以及直接跨表写入被拒绝。除此之外，不允许任何跨 Context 事务或表直读例外。
...
12. 除已登记 `poolai_quota_reserve` 与 `poolai_validate_group_activation` 外出现跨 Context SQL/DbContext 直读；两者读取超出固定 canonical 字段白名单、写入其他 Context，或 quota reserve 未遵守数据库锁序。
