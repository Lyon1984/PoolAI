...
投递语义为 at-least-once，不承诺全局顺序。需要顺序的消费者只能按已声明的 topic/aggregate sequence 处理；inbox receipt、投影写入和 checkpoint 前进必须在消费者的同一 UoW。重复消息结果不变，同 sequence 不同 payload hash 必须告警并停止该分区；未知 `schema_version` 不得静默丢弃。按 Proposed [`ADR 0012`](adr/0012-freeze-quota-event-replay-ordering-and-projection-boundary.md)，`poolai.quota.v1` 的分区固定为 Group，逻辑 sequence 是 replay 保留的 `source_event_sequence`；物理 Outbox `event_sequence` 只承担 delivery/Inbox identity。该区分在 ADR 获永久批准前仍是候选，不能用于 M3-E4 sign-off。
...
按 Proposed ADR 0012，通用 Publisher 只 claim 已显式登记 consumer route 的 topic；对同一 quota Group，只允许最小未完成 `source_event_sequence` lineage 前进。dead lineage 只阻断该 Group，保留原 source sequence 的 pending replay 可从原逻辑位置恢复，其他 Group 可继续。Publisher 在 claim 事务提交后调用 durable consumer；只有 strict codec、fact-reference 校验和 consumer 单 UoW 成功或精确 Inbox duplicate 后，才以独立 owner/generation CAS 标为 `published`。未知 major、Envelope/payload/fact 不一致使 consumer 回滚、物理消息进入 dead并发 P0；发布确认前崩溃允许重复投递。该 publication guard 不把 consumer 事务并入 claim UoW，也不把已提交 producer 业务事务回滚。
