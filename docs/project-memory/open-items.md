...
- M1-E4 architecture and database approvals are independently governed by [`ADR 0006`](../architecture/adr/0006-register-group-subscription-lifecycle-fence.md) and [`0007_group_subscription_m1_e4.sql`](../database/0007_group_subscription_m1_e4.sql) plus its release-manifest entry. Tests may validate either candidate, but any record still `Proposed` or unsigned requires explicit `@Lyon1984` approval and its own permanent Issue #44 evidence; neither approval substitutes for the other, and no remote database mutation is authorized by these candidates.
