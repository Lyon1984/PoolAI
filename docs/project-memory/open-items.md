...
- [`ADR 0004`](../architecture/adr/0004-keep-subscription-template-catalog-admin-only.md) proposes resolving the M1-E4 User Template-summary conflict by keeping the Template catalog Admin-only and exposing only the caller's assigned `plan_name` snapshot through the existing User Subscription projections. The implementation may proceed against that fail-closed candidate, but the ADR/RBAC increment must remain proposed until `@Lyon1984` explicitly approves it before merge.
- [`ADR 0005`](../architecture/adr/0005-open-exact-v1-compatibility-window-for-admin-list-validation.md) and [`compatibility-windows-v1.json`](../contracts/compatibility-windows-v1.json) register the exact M1-E4 candidate that adds the existing 400 `BadRequest` response to three Admin pagination GET operations. It remains `proposed`, has no approval evidence, and intentionally makes the Git-base compatibility command fail pending approval; only an explicit `@Lyon1984` approval followed by a real permanent Issue #44 comment may atomically move the registry/ADR/evidence markers to accepted.
- [`ADR 0006`](../architecture/adr/0006-register-group-subscription-lifecycle-fence.md) proposes the complete three-family cross-context PostgreSQL read/row-lock allowlist, including the pre-existing narrow route-identity locks and the M1-E4 Group–Subscription lifecycle fence. It remains `Proposed`; its tests may validate the candidate, but only explicit `@Lyon1984` approval may make it effective or complete the M1-E4 architecture sign-off.
- [`0007_group_subscription_m1_e4.sql`](../database/0007_group_subscription_m1_e4.sql) and its release-manifest entry are an unsigned M1-E4 database increment. Local PostgreSQL 18 migration, permission, lifecycle-fence, and repository evidence do not replace explicit `@Lyon1984` approval and a permanent Issue #44 sign-off comment; no remote database mutation is authorized by this candidate.
