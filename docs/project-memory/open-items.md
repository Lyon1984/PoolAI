...
- ADR 0008/精确十一诊断 OpenAPI window 与 forward migration 0009 已分别取得 `@Lyon1984` 的永久批准证据，但新增真实 PostgreSQL 18 Testcontainers 在本地因无可用 Docker daemon 尚未执行；PR/CI 必须完成空库 0001..0009、函数/constraint/ACL 与 Unicode 边界验证。任何获准的远程执行前还必须只读预检既有 `api_keys.name/revoke_reason`，因为 0009 对不合规旧行会原子失败且不会自动 trim/改写；当前签核不授权远程 migration 或数据修复。
