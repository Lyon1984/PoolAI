...
- ADR 0009 remains `Proposed`. The current local M2-E1 persistence/rewrap candidate now has real PostgreSQL 18 persistence/replacement, deterministic concurrent CAS, bounded crash/retry, redacted failure-fact and least-privileged execution evidence, but it must remain unsigned until `@Lyon1984` explicitly approves the exact candidate after review; no current comment or baseline signature accepts it.
- Forward migration `0010_supply_account_credentials_m2_e1.sql` remains an independently unsigned candidate at SHA-256 `0de4b5e38ab8dee9f7d5b222e729947e825934241f2c9b8ae2598f89267174c0`. Its API/Worker entry-point ABI, trigger ownership and ACLs are locally verified, but database approval must be requested separately after the candidate commit is pinned. No remote migration, data repair, rewrap execution, key retirement, or deployment is authorized.
