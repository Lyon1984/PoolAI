...
- M1-E5 runtime/database implementation is gated on explicit `@Lyon1984` approval of proposed [ADR 0007](../architecture/adr/0007-freeze-api-key-lifecycle-and-validation-contract.md) and its exact thirty-four-diagnostic OpenAPI window. Until a permanent Issue #44 approval comment is published and read back, the registry must remain `proposed`, compatibility must fail `pending approval`, and migration 0008 must not be treated as an accepted contract.
