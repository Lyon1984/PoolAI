...
- GitHub readback confirms the authorized private `Lyon1984/PoolAI` repository, default `main` branch, and bootstrap commit. Its initial `quality-gate` passed verify, dynamic Compose, and the three-browser smoke matrix; the security runs passed full-history secret/dependency/license checks plus source/image SBOM and image scans. PR #1 verified the CodeQL `actions: read` correction through analysis and SARIF export, then GitHub rejected the upload because Code Scanning is not enabled for the private repository; Code Security entitlement/enablement is now the explicit external blocker. PR quality logs also exposed that the runner lacked `rg` while both coverage-integrity and forbidden-scope commands suppressed that failure; the branch replaces them with locked-Node, fail-closed scanners and still requires target rerun evidence.

## Not yet implemented or verified

- The authorized private GitHub repository, initial `main` publication, full-history Gitleaks, browser matrix, SBOM/image scans, and CI-hosted dynamic Compose evidence now exist. PR changed-line coverage correctly reported not evaluated because PR #1 contains no production source files; a source-changing PR must still prove the non-zero path. The fail-closed coverage-integrity/forbidden-scope rerun, private-repository Code Security entitlement/enablement, required checks, and branch protection remain pending
