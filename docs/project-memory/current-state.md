...
- GitHub readback confirms the authorized public `Lyon1984/PoolAI` repository, default `main` branch, and bootstrap commit. PR #1 passed `verify`, Chromium smoke, dynamic Compose, full-history secret/dependency/license checks, source/image SBOM and image scans, and CodeQL. Its logs exposed missing-`rg` false-pass paths in coverage-integrity and forbidden-scope; the PR replaces both with locked-Node, fail-closed scanners, and the final target log contains both success records with zero command-not-found output. CodeQL uploaded 166 rules with zero results, and the open-alert API readback is empty.
