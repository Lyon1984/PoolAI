...
- The first post-merge full default-branch CodeQL analysis uploaded successfully but surfaced four open alerts: one High `js/file-system-race` in traceability evidence handling and three Medium download/file-flow findings in the checksum-verifying CI installers. A follow-up local change replaces the real check/use race with canonical, `O_NOFOLLOW`, single-descriptor `fstat`/read handling and adds runtime lock/origin/size/checksum enforcement plus offline negative probes for all three release installers; the full local quality gate passes, while final disposition still requires protected-PR and default-branch CodeQL readback.
...
M0 remains active. The M0-E3 transaction/idempotency/messaging baseline, M0-E4 local runtime compatibility path, repository-owned PR/release gates, repository-side DEC/AC traceability, a complete 38-Epic task-import preview, the authorized public GitHub bootstrap, target workflow execution, and protected `main` are implemented or externally verified. The next items are to close or evidence-classify the four default-branch CodeQL alerts through the protected path, exercise changed-line coverage on a production-source PR, import the preview into the real task system and read back IDs/owners, complete DEC/database/OpenAPI sign-off, and conduct the M0 exit review. M1 feature implementation must wait for M0 exit sign-off.
