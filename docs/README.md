...
2. 由平台先预置 `poolai_runtime_owner NOLOGIN`、`poolai_api`、`poolai_worker`，再由具备 owner 切换/授权能力的 Migrator 按 manifest 顺序执行 `0001_baseline.sql`、`0002_quota_functions.sql`、`0003_runtime_permissions.sql` 及后续已登记前向 migration（当前增量依次为 `0004_identity_m1_e1.sql`、`0005_identity_m1_e2.sql`、`0006_identity_m1_e3.sql`、`0007_group_subscription_m1_e4.sql`、`0008_identity_api_keys_m1_e5.sql`、`0009_identity_api_key_text_validation_m1_e5.sql` 和已独立数据库签核的 `0010_supply_account_credentials_m2_e1.sql`），并生成 EF Core 映射；0003 不创建角色，API/Worker 不得执行迁移，也不得让 EF migration 反向改变 baseline。已接受的 [`ADR 0009`](architecture/adr/0009-introduce-shared-secrets-envelope-runtime.md) 架构批准与 [migration 0010 独立数据库批准](https://github.com/Lyon1984/PoolAI/issues/44#issuecomment-5117955336) 保持分治；后者精确冻结候选 `6278efb09a308a011fc9c7c6175a269fba9faedd`、SQL SHA-256 `0de4b5e38ab8dee9f7d5b222e729947e825934241f2c9b8ae2598f89267174c0` 与 manifest `10..10`。两条批准均不授权远程执行、Worker/rewrap/key operation、PR Ready/merge、部署或 M2-E1/发布验收。
