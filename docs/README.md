...
- [`ADR 0005`](architecture/adr/0005-open-exact-v1-compatibility-window-for-admin-list-validation.md) 与独立的 [`compatibility-windows-v1.json`](contracts/compatibility-windows-v1.json) 登记 M1-E4 三个 Admin 分页 GET 候选窗口，不修改或复用 ADR 0003 的 reset 记录。候选仅在 base/target 摘要和完整诊断精确匹配时可被识别；`proposed` 状态必须以 pending approval 失败，不构成豁免。只有在 `@Lyon1984` 明确批准并实际形成 Issue #44 永久评论后，registry/ADR 同时转为 `accepted`/`Accepted` 且登记同一非占位 approval evidence URL，机器门才能精确放行。合并后 target 立即恢复为普通 `/v1` 比较基线，该窗口不能用于其他 base 或 target。
