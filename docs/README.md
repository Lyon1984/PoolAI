...
- [`ADR 0011`](architecture/adr/0011-freeze-upstream-health-and-breaker-boundary.md) 是尚待 `@Lyon1984` 永久批准的 M2-E4 架构候选：它只允许 Worker 加载 Routing 的 breaker/probe + Supply Health coordinator graph，禁止完整 Router/affinity/Group RPM/Gateway/Adapter；新 Account unknown 走受控 `/models` 验证而非 half-open，并冻结连接期 SSRF、内部 readiness summary 与三个 breaker Lua 的所有权/协议边界。其 Proposed 状态不授权 Host graph、Lua/manifest、数据库、远程 upstream/credential、部署、合并、M2-E4 或 M2 Exit。
...
Proposed ADR 0011 对 D-024/D-025/D-029 的 M2-E4 细化在永久批准前不生效；批准后也只扩大 Worker 的 Routing health-only 注册，不改变 Host 数量、完整 Routing/Gateway 加载边界、表 owner、累计 quota 或 retry/failover owner。
