...
- [`ADR 0007`](architecture/adr/0007-freeze-api-key-lifecycle-and-validation-contract.md) 当前保持 **Proposed**，并为 M1-E5 原子 rotate、versioned pepper、canonical CIDR、credential response shape/强 ETag 与精确三十四条兼容诊断登记候选窗口。它在 `@Lyon1984` 发布永久批准评论、ADR/registry 原子转为 Accepted/accepted 前不是有效豁免；机器兼容门必须以 `pending approval` 失败，且该候选不授权 migration 0008、远程数据库、部署或发布。
