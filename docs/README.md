...
- [`ADR 0008`](architecture/adr/0008-freeze-api-key-unicode-scalar-text-contract.md) 与第三条 compatibility-window 记录目前只是 **Proposed**/`proposed` 候选：它把 API Key name/reason 收敛到冻结的 Unicode scalar/White_Space/control 规则，并精确绑定 base `f91b11950c117c382d2a6b96ac531fa864124101`、target OpenAPI SHA-256 `1c9dee2fe48cd3e2f0fa5a00805e07e21d303b5a4fa070faeab66f3be6132141` 与十一条诊断。机器门确认完整集合后仍必须以 `pending approval` 失败；只有 `@Lyon1984` 在 Issue #44 明确批准并回写永久证据后才能转为 Accepted/accepted。候选不改写已签 0008，也不授权 forward 0009 的远程执行、部署或发布。
