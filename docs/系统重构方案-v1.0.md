...
当前仓库已具备锁定的 .NET/Vue workspace、独立 Api/Worker/Migrator Host、依赖架构门、契约生成与校验、真实 PostgreSQL/Redis 集成测试、两条可执行 CI workflow 和七服务本地 Compose。单一 release manifest 现已同时约束 Migrator、Api readiness 与 Worker 启动门；数据库历史 checksum、PostgreSQL/Redis major、Redis key schema/TIME/脚本加载均 fail-closed。最近验证为 31 个项目零 warning/error、前端 8 项测试、.NET 203 项测试且零跳过；六个 xUnit 项目统一启用 `failSkips` 并由动态 Skip 反向探针证明门禁有效。覆盖率包含 25/25 个生产程序集，整体行覆盖率 79.64%、前端 92.86%、Domain/Application 行与分支均 100%；Chromium smoke、脱敏 secret scan、依赖/许可证扫描、三镜像非 root 检查，以及 Migrator、Api/Worker compatibility gate、Worker 零重启稳定运行和 NTP readiness 动态联调全部通过。因此 Codex 已满足继续实施 M0 的工程条件，但尚未满足“完成 M0”或进入 M1 的条件。

M0-E3 的生产 UoW/repository、完整 command idempotency、outbox/inbox/email claim 与接管、Worker advisory lock、提交点故障矩阵，以及 M0-E4 的本地 schema/Redis compatibility gate 均已有自动化与真实容器证据。Coverage、browser smoke、secret/CodeQL/license/SBOM/image scan、CI 动态 Compose、42 个 DEC/45 个 AC 到主 WS/直接 Epic/契约/本地证据或命名计划测试的仓库实现，以及覆盖全部 38 个 M0–M7 Epic 的任务导入预览已经闭合。目标公开 GitHub 的 full-history secret scan、browser matrix、SBOM/image scan、动态 Compose 与 CodeQL workflow 均已通过。PR #1 的质量日志暴露并修复了缺少 `rg` 被 coverage-integrity 与 forbidden-scope 吞掉的误通过风险；最终 PR 与默认分支日志确认两个 locked-Node fail-closed scanners 均真实执行且没有 command-not-found，PR #1 已 squash-merge。其增量 CodeQL 分析为零结果，但首次合并后默认分支全量分析随后发现一条真实文件竞态和三条校验式 CI 下载数据流告警；前者已有单描述符本地修复，后者已有运行时锁/来源/体积/摘要约束与负向探针，仍须经受保护 PR、默认分支复扫和证据分类闭合，不能宣称公开告警为空。`main` 已要求六项严格检查、PR 与对话解决，对管理员同样生效并禁止强推和删除。PR #1 未含生产源码，changed-line coverage 因而正确报告未评估。尚未闭合的是上述 CodeQL 告警处置、后续生产源码 PR 的非零 changed-line coverage，以及真实任务 ID/owner、DEC/数据库/OpenAPI 签收和 M0 exit review 等外部证据。

### 13.2 立即执行顺序

1. 在首个生产源码 PR 验证非零 changed-line coverage，并持续保留现有六项 required checks 与 `main` 分支保护。
