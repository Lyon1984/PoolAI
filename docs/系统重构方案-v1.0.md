...
M0-E3 的生产 UoW/repository、完整 command idempotency、outbox/inbox/email claim 与接管、Worker advisory lock、提交点故障矩阵，以及 M0-E4 的本地 schema/Redis compatibility gate 均已有自动化与真实容器证据。Coverage、browser smoke、secret/CodeQL/license/SBOM/image scan、CI 动态 Compose、42 个 DEC/45 个 AC 到主 WS/直接 Epic/契约/本地证据或命名计划测试的仓库实现，以及覆盖全部 38 个 M0–M7 Epic 的任务导入预览已经闭合。目标 GitHub 首次 `quality-gate`、full-history secret scan、browser matrix、SBOM/image scan 与动态 Compose 已通过；CodeQL 已完成分析和 SARIF 导出，但首次运行暴露出私有仓库缺少 `actions: read`，修复后的首次 PR 运行仍需验证权限与 entitlement。尚未闭合的是有真实 PR base 的 changed-line coverage、CodeQL 成功证据、required checks/branch protection，以及真实任务 ID/owner、DEC/数据库/OpenAPI 签收和 M0 exit review 等外部证据。

### 13.2 立即执行顺序

1. 通过首个 PR 验证 CodeQL 私有仓库权限修复、entitlement 与 changed-line coverage，归档已通过的 full-history secret scan、browser matrix、SBOM/image scan 与动态 Compose 目标环境证据；随后配置 required checks 和分支保护，禁止再次直接推送 `main`。
