...
| PoolAI 当前仓库状态 | 工作区内已经存在且可验证的资产 | M0-E3/E4 与仓库负责的 PR/发布门已通过本地质量、安全、浏览器、真实容器和 Compose 验证；目标 CI 与签收证据仍未闭合 |
...
| WS-03 | Identity 与 SubscriptionAccess | bootstrap、登录/TOTP、Redis 登录失败限流、Session、重置密码、RBAC、API Key、Template、canonical Subscription | WS-01/02 | 身份撤销、并发 refresh、登录限流 fail-closed/防枚举、Key 一次回显、订阅状态机和审计通过 |
...
7. User 登录并从有效订阅选择 Group 创建 API Key；完整 Key 只显示一次。
...
- `poolai_quota_reserve` 必须在自己的短事务中按数据库契约的固定锁序重新锁定并校验 Identity、Subscription、Group、`group_supply_configurations` 与其他 Supply canonical 行，确认请求 Channel 等于当前 configured Channel，所有等待后重新读取 PostgreSQL clock，再写 GroupQuota 事实。它与 `poolai_validate_group_activation` 是仅有的跨 Context 数据库窄例外；都只能读取白名单字段，不能写其他 Context。旧 activation evidence 或 Redis snapshot 均不能代替该强读。
...
- API Key 只保存 HMAC-SHA256 digest、prefix 和 pepper version，完整值只在创建响应中返回一次。
...
| WS-03 Identity 与 SubscriptionAccess | M1-E1–E5 | DEC-003–008/018–020/026/028/031/033；AC-001–009/029/033–035/040 | [`openapi-v1.yaml`](contracts/openapi-v1.yaml)、[`error-catalog.md`](contracts/error-catalog.md)、执行规格第 3–4 节 | Unit/Contract/Integration/E2E；会话并发、Redis 登录限流/fail-closed/防枚举、撤权强读、Key 一次回显、email outbox 和审计 |
...
当前仓库已具备锁定的 .NET/Vue workspace、独立 Api/Worker/Migrator Host、依赖架构门、契约生成与校验、真实 PostgreSQL/Redis 集成测试、两条可执行 CI workflow 和七服务本地 Compose。单一 release manifest 现已同时约束 Migrator、Api readiness 与 Worker 启动门；数据库历史 checksum、PostgreSQL/Redis major、Redis key schema/TIME/脚本加载均 fail-closed。最近验证为 31 个项目零 warning/error、前端 8 项测试、.NET 203 项测试且零跳过；六个 xUnit 项目统一启用 `failSkips` 并由动态 Skip 反向探针证明门禁有效。覆盖率包含 25/25 个生产程序集，整体行覆盖率 79.55%、前端 92.86%、Domain/Application 行与分支均 100%；Chromium smoke、脱敏 secret scan、依赖/许可证扫描、三镜像非 root 检查，以及 Migrator、Api/Worker compatibility gate、Worker 零重启稳定运行和 NTP readiness 动态联调全部通过。因此 Codex 已满足继续实施 M0 的工程条件，但尚未满足“完成 M0”或进入 M1 的条件。

M0-E3 的生产 UoW/repository、完整 command idempotency、outbox/inbox/email claim 与接管、Worker advisory lock、提交点故障矩阵，以及 M0-E4 的本地 schema/Redis compatibility gate 均已有自动化与真实容器证据。Coverage、browser smoke、secret/CodeQL/license/SBOM/image scan、CI 动态 Compose、42 个 DEC/45 个 AC 到主 WS/直接 Epic/契约/本地证据或命名计划测试的仓库实现，以及覆盖全部 38 个 M0–M7 Epic 的任务导入预览已经闭合。目标 GitHub 的 full-history secret scan、browser matrix、SBOM/image scan 与动态 Compose 已通过；PR #1 证明 CodeQL `actions: read` 修复有效，但私有仓库未启用 Code Scanning，上传仍受 entitlement/enablement 阻塞。该 PR 未含生产源码，changed-line coverage 因而正确报告未评估；同时质量日志暴露缺少 `rg` 被 coverage-integrity 与 forbidden-scope 吞掉的误通过风险，仓库已改用锁定 Node 的 fail-closed scanners，仍待目标复跑。尚未闭合的是非零 changed-line coverage、CodeQL 成功证据、required checks/branch protection，以及真实任务 ID/owner、DEC/数据库/OpenAPI 签收和 M0 exit review 等外部证据。

### 13.2 立即执行顺序

1. 在 PR #1 复跑 fail-closed coverage-integrity 与 forbidden-scope；取得并启用私有仓库 Code Security entitlement 后复跑 CodeQL，并在首个生产源码 PR 验证非零 changed-line coverage。归档已通过的 full-history secret scan、browser matrix、SBOM/image scan 与动态 Compose 目标环境证据；随后配置 required checks 和分支保护，禁止再次直接推送 `main`。
2. 将仓库内已校验的 DEC/AC 映射导入真实任务系统并分配 ID/owner，完成 DEC/数据库/OpenAPI 签收及 M0 exit review。
3. M0 全绿后，按 M1 → M2/M3 → M4 → M5 → M6 主链推进。
