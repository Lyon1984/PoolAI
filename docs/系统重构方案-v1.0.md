...
- Group RPM 只对通过鉴权且完成 canonical Key/User/Subscription/Group 强读的 Responses/Chat 模型 POST 按入站 request 计数一次；limit 只能来自 GroupQuota-owned exact `runtime_policy={"schema_version":1,"requests_per_minute":N}`，`N` 默认 `6000`、范围 `1..1,000,000`。Group create omission用默认值，response 始终返回有效值，PATCH omission 保持不变，任何 PATCH 提交 `requests_per_minute` 必须同时提交 reason。failover 不重复计数，Models 和 Usage 不进入 RPM；Redis/配置/Supply/Subscription 不能成为 limit authority，既有 Redis key/Lua/TTL/`TIME` ABI 不变。
...
- 已批准的 [`ADR 0015`](architecture/adr/0015-freeze-m4-e1-gateway-admission-and-upstream-handoff.md) 只登记精确新 Family C identity `poolai_group_update_v2 -> subscriptions(group_id,status,expires_at)`，要求复用旧 update archive branch 的同一 lock/order/fields/post-wait clock，且不把 `poolai_group_create_v2` 加入 allowlist。其 exact OpenAPI compatibility-window 与 migration 0019 也已分别完成独立批准，永久证据见 [`docs/README.md`](README.md)；migration preflight 只接受升级前精确 `{}`，任意非空 `runtime_policy`（即使形似未来两键对象）使整笔 migration 在 backfill/DDL 前原子失败。ADR/窗口/数据库三项批准都不授权远程 migration、Redis、credential、upstream 或部署操作。
- API Key CIDR 客户端地址只从 socket trust anchor 解析：`Gateway:Ingress:TrustedProxyCidrs=[]` 默认不信任代理，最多 64 个 canonical CIDR；`Gateway:Ingress:ForwardedForLimit=1`，范围 1..8。直接 peer 不可信就忽略 forwarding header；可信时只接受一个不超过 1024 UTF-8 bytes 的 `X-Forwarded-For`，bare IP 从右向左按 trusted chain 剥离，malformed 或 CIDR mismatch 统一映射既有 `invalid_api_key`。不读取 `Forwarded`/`X-Real-IP`，raw IP/header 不进入日志、trace 或 metric。
- `conservative-v1` estimate 只对固定 compact normalized payload 做 `estimated_input=max(1, UTF8 bytes + 64)`；output 取显式 `max_output_tokens`/`max_completion_tokens`，否则为 4096；以 checked `BigInteger` 相加并受单 attempt 2,000,000 上限约束，超限在 route/lease/reservation 前返回既有 422。Adapter usage 则从原始 JSON integer text 直接进入完整 `BigInteger` usage（input/output/cache-read/cache-creation/thinking），先无损 settle；仅公开 safe-number 映射超界时用既有 `upstream_usage_out_of_range`，78 位溢出事务回滚并发 `token_numeric_overflow` P0，绝不 clamp。
- Selected route 只带非秘密 IDs、provider、client/upstream model、canonical base URI、capabilities、Supply/Channel/Account versions 与 `credential_revision`。Supply 提供 revision-fenced one-use credential lease，Gateway 在 reservation 后、dispatch fence 前将其包装为 Gateway.Abstractions opaque zeroizing handle；Adapter 仍只依赖 Gateway.Abstractions。只有 ADR 0010/0011 连接期 DNS/egress/Host/SNI/certificate fence 通过后才能附加 Authorization，route/DTO/log/trace 不得含 secret/envelope。
- 只有 `not_started` reservation 可以零消耗 release/expire；dispatch 后的歧义结果按 estimate 保守核销，后得真实值用 adjustment 修正。
- 上游调用存活期间，Routing 每 20 秒按 Redis contract 续租 Account lease：typed `Lost` 立即取消上游，连续两次 coordination failure 也取消；GroupQuota 按数据库 contract 续租 reservation。两者汇入同一 attempt cancellation，但时钟、间隔和所有权不能混用。续租不能突破流/非流绝对生命周期上限；任一硬 lease 失效或到达上限都要取消上游，最多执行契约规定的 15 秒 bounded drain，再按 dispatch phase 用已知 usage/保守 estimate settle 或仅在 pre-dispatch release，最后 zeroize credential 并释放 Account lease。任何 renew/cancel/drain/settle/release 等待都不持有数据库事务。
...
5. 对账任务按 [`ADR 0013`](architecture/adr/0013-freeze-quota-reconciliation-layers-checkpoint-alignment-alerting-and-recovery-boundary.md) 分别核对 GroupQuota 权威账本/事实、Usage logical source checkpoint 对齐投影和 Operations 交付健康；不得把当前 ledger 与滞后 projection 直接比较。
6. 权威差异立即触发 P0 并由 Admin 通过既有 Group lifecycle 显式隔离；detector 不写权威账本。Usage-owned derived projection 只能有界 replace/rebuild，恢复前后都保留结构化证据并重新核对。
...
| M1 | Identity、RBAC、Redis 登录限流、API Key、GroupQuota-owned Group 基础（DTO/Command 不含 Channel/binding）、Template、canonical Subscription | M0 绿色 | 身份/会话/撤销/登录限流 fail-closed 与防枚举、订阅状态机、Group ETag、审计及当前 M1 operation RBAC 自动化通过；无生产 Supply Configuration 写路径；有效 Subscription/API Key 只能使用第 8.1 节限定的受控 M2 Supply 测试接缝验收，不构成 M2 实现或部署能力 |
| M2 | secret envelope、显式 Provider、独立 Group Supply Configuration/ETag、Channel/Account、映射、绑定、opaque activation evidence/编排、健康、Account lease acquire/renew/release、RPM、breaker 和同 Group 调度 | M1 Policy 和 Group 基础可用 | 生产 Supply API 与 readiness Adapter 已替换 M1 测试接缝，并从空库/bootstrap Admin、不经特权业务表预置闭合 Account/Channel/Configuration/binding → Group activation → Subscription → Key；双 ETag/幂等隔离、Orchestration/写 owner、evidence/race、供应安全、逐 attempt canonical recheck、长请求 lease、多实例协调、完整 Redis 故障矩阵和同 Group 约束通过 |
...
| M4 | Responses、Chat、Models、SSE、Gateway Process Manager 和 failover；M4-E1 先闭合 per-Group RPM、trusted proxy/CIDR、deterministic estimate、lossless usage 与 revision-fenced route/credential handoff | M1/M2/M3 退出；ADR 0015、精确 OpenAPI window、migration 0019 各自获批并经受保护主干同步 | OpenAI/Codex 垂直切片的合同、E2E、流、取消和故障矩阵通过 |
...
- M1 Exit 的 Supply 接缝只存在于自动测试：它仅可在公开 API 创建 Group 后预置 ADR 0001 所属的 M2-owned PostgreSQL 前置事实，并仅替换尚未实现的 M2 `IGroupSupplyReadiness` Adapter；Adapter 必须读取真实预置事实，不得恒定返回 ready。M1-owned 事实和端口仍全部走公开 HTTP、生产注册、真实 PostgreSQL/Redis。该接缝不得进入生产程序集或部署配置，M2 Exit 必须以公开 Supply API 和生产 Adapter 将其移除。
...
| quota 不变量、重复核销或跨 Group 事实 | 全平台停止新模型准入和 quota 管理写；受影响 Group 另由 Admin 以 reason/ETag/idempotency 显式置为 disabled；保留证据并 drain 当前 attempt | detector 不自动改账或自动切 lifecycle；不假设应用回滚可修复，使用正式 adjustment 或恢复/fix-forward，绝不删账本事实 | 权威、checkpoint-aligned projection 与交付健康三层验证为 0 差异，Admin 显式重新启用并完成 P0 复盘 |
...
| WS-06 Gateway 与 OpenAI Adapter | M4-E1–E5 | DEC-015–017/022–025/028/039/041；AC-012/015/016/020/021/028/029/039/042/043/045 | [`ADR 0015`](architecture/adr/0015-freeze-m4-e1-gateway-admission-and-upstream-handoff.md)、[`openapi-v1.yaml`](contracts/openapi-v1.yaml)、[`fixtures/`](contracts/fixtures/)、错误目录、Design Pattern/Redis/数据库契约 | Adapter/Contract/Architecture/Integration/E2E/Fault；per-Group RPM、trusted proxy/CIDR、deterministic estimate、lossless usage、credential/SSRF handoff、lease cancellation、phase、SSE 半包/断流/取消、safe retry/failover、bulkhead |
