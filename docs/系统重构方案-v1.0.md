...
| PoolAI 当前仓库状态 | 工作区内已经存在且可验证的资产 | M0-E1–E4 已形成完成证据并关闭；仓库负责的 PR/发布门已通过本地及目标 CI 的质量、安全、浏览器、真实容器和 Compose 验证；38 个 Epic 的 GitHub Issues/owner 已回读，DEC-001..042、数据库、OpenAPI 与 M0 Exit 已由 Issue #44 的四条独立永久评论签收。M1 已获准开始，M1-E1 已形成通过本地全仓质量门的实现候选，但不表示 M1 里程碑或 Release 1 已验收 |
...
2. 从 M1-E1 用户与 bootstrap 开始，随后按 M1 → M2/M3 → M4 → M5 → M6 主链推进；不得把开发启动授权表述为实现或验收完成。
