...
| PoolAI 当前仓库状态 | 工作区内已经存在且可验证的资产 | M0-E1–E4 已形成完成证据并关闭；仓库负责的 PR/发布门已通过本地及目标 CI 的质量、安全、浏览器、真实容器和 Compose 验证；38 个 Epic 的 GitHub Issues/owner 已回读，DEC-001..042、数据库、OpenAPI 与 M0 Exit 已由 Issue #44 的四条独立永久评论签收。M1 已获准开始，M1-E1 与 M1-E2 已形成通过本地全仓质量门的实现候选，但不表示 M1 里程碑或 Release 1 已验收 |
...
7. User 登录后只读取本人获授 Subscription 及其 `plan_name` 快照，不浏览独立 Template 目录或自助申请；再从有效订阅选择 Group 创建 API Key，完整 Key 只显示一次。
...
当前仓库已具备锁定的 .NET/Vue workspace、独立 Api/Worker/Migrator Host、依赖架构门、契约生成与校验、真实 PostgreSQL/Redis 集成测试、两条可执行 CI workflow 和七服务本地 Compose。单一 release manifest 现已同时约束 Migrator、Api readiness 与 Worker 启动门；数据库历史 checksum、PostgreSQL/Redis major、Redis key schema/TIME/脚本加载均 fail-closed。最近验证为 31 个项目零 warning/error、前端 8 项测试、.NET 712 项测试且零跳过；六个 xUnit 项目统一启用 `failSkips` 并由动态 Skip 反向探针证明门禁有效。覆盖率包含 25/25 个生产程序集，整体行覆盖率 90.41%、前端 92.86%、Domain/Application 行 97.81%/分支 87.53%、认证行 94.73%/分支 83.34%、秘密处理行 88.11%/分支 75.42%；Chromium smoke、脱敏 secret scan、依赖/许可证扫描、三镜像非 root 检查，以及 Migrator、Api/Worker compatibility gate、Worker 零重启稳定运行和 NTP readiness 动态联调的既有证据继续有效。因此 M0 工程基线与开发启动门已通过，M1-E1/M1-E2 已形成本地验证候选；这仍不表示 M1 里程碑或 Release 1 已验收。
...
2. M1-E1 用户/bootstrap 与 M1-E2 Session 安全已形成本地验证候选；M1-E2 仓库交付证据闭合后按依赖进入 M1-E3 canonical 授权，再沿 M1 → M2/M3 → M4 → M5 → M6 主链推进。不得把本地候选或开发授权表述为里程碑/发布验收完成。
