...
- 候选 [`ADR 0015`](architecture/adr/0015-freeze-m4-e1-gateway-admission-and-upstream-handoff.md) 仍待 Issue #44 明确批准；它只候选登记精确新 Family C identity `poolai_group_update_v2 -> subscriptions(group_id,status,expires_at)`，要求复用旧 update archive branch 的同一 lock/order/fields/post-wait clock，且不把 `poolai_group_create_v2` 加入 allowlist。其 exact OpenAPI compatibility-window 与 migration 0019 需要独立签核；migration preflight 只接受升级前精确 `{}`，任意非空 `runtime_policy`（即使形似未来两键对象）使整笔 migration 在 backfill/DDL 前原子失败。ADR/窗口/数据库三项批准都不授权远程 migration、Redis、credential、upstream 或部署操作。
