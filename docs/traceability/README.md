...
[`delivery-epics.json`](delivery-epics.json) is the task-import registry for all 38 M0–M7 Epics. It records the primary/supporting WS ownership frozen in the system plan and keeps external task IDs and owners empty until a real task system returns them. The deterministic exporter merges this registry with the authoritative backlog and Release 1 traceability without turning generated output into another contract source.
...
Local evidence must declare either `kind: test`, which resolves to an actual xUnit `[Fact]`/`[Theory]` method, or `kind: quality-gate-command`, which resolves to an exact command executed by the repository quality gate. The validator parses the authoritative WS applicability matrix and locks the reviewed ID/WS/Epic map against silent remapping. External task IDs, owners, review signatures, target-CI evidence, and M0 exit approval stay empty until the corresponding external system produces auditable references; format checks cannot replace external readback or human approval. The target-CI gate is now verified by PR #4, whose protected check readback covers all six required jobs and the first non-zero production-source changed-line run; task-system, review, and exit gates remain pending.
...
Task-import output is always a preview while `externalEvidence.taskSystem` is pending. It contains no invented task ID or owner. Six Epics intentionally have no direct R1.1 DEC/AC association: M3-E5, M5-E6, and the four post-GA M7 Epics; the exporter locks that set so future accidental mapping loss fails validation.
