...
| Group quota 越权变更拒绝 | `rate:quota-denial:v1:{actor_user_id}:{minute_epoch}` / String integer | 当前 Redis 分钟内同一已认证非 Admin 用户对 quota adjust/reset 的尝试总数 | 120 s | limit=`Quota:DeniedMutationAttemptsPerMinute`（默认 5，范围 1..20）；全部 Group 与 adjust/reset 共用 actor 窗口；超限返回 `429 rate_limit_exceeded` 且不写拒绝审计，异常返回 `503 coordination_unavailable` 且不进入 PostgreSQL |
...
Group quota 的 Admin-only adjust/reset 在已认证 actor、非空 Group 路径和 operation 通过授权预检的结构校验后处理非 Admin 拒绝。Admin 不调用本 limiter；Operator、Auditor、User 在读取请求 body、`Idempotency-Key`、`If-Match` 或开启 PostgreSQL UoW 前，以 canonical 小写 UUID `actor_user_id` 调用同一 `rate:quota-denial:v1` fixed-window。key 不含 Group、operation、JWT/session、Token version 或 IP，因此同一 actor 不能通过交替 adjust/reset、枚举 Group 或更换会话绕过；不同 actor 使用不同 Cluster slot。允许时才追加恰好一条既有 append-only denial audit 并返回 `403 role_required`。超限使用 `ceil(retry_after_ms/1000)` 返回 `429 rate_limit_exceeded + Retry-After`，且不得写 audit/quota/event/outbox/idempotency；Redis 超时、断开、脚本重载失败或不兼容结果返回 `503 coordination_unavailable + Retry-After: 1`，同样不得打开数据库 UoW，也不得回退进程内计数。超限响应由 HTTP telemetry 观察，不能为每次 429 再写持久审计而重新形成 write amplification。

拒绝审计在 transport body/header 校验前只记录 `Idempotency-Key` 的安全材料：header 不存在或为空记录 `missing`（HTTP stack 可把空 header 规范化为不存在，transport 同样返回 428）；单值含非法字符或超过 128 字符记录 `invalid`；多值记录 `multiple`；这三类的 hash 均为 null。恰好一个满足 ASCII 1..128 的合法值记录 `valid`，并只写使用 `Idempotency:RequestHashPepper` 和独立 domain 的 HMAC-SHA256 小写 hex。原始 key 不得进入审计、日志、metrics 或 Redis key/value。该审计 HMAC 不参与命令幂等判定，也不改变合法 Admin 命令的 idempotency scope。
