...
10. `0010_supply_account_credentials_m2_e1.sql`：M2-E1 候选前向增加 Account credential 独立 revision、Envelope v1 存储结构 guard、API create/replace 与 Worker keyset selector/确定性 CAS rewrap 窄入口；收回 API 对 Account 的直接 INSERT/UPDATE，且不改写 0001–0009。
...
0010 当前只是 M2-E1 数据库候选，没有数据库签核、远程执行、部署或发布授权。它必须以候选 commit、SQL 精确 SHA-256、manifest `10..10`、真实 PostgreSQL 18 的空库/并发/角色证据单独提交 `@Lyon1984` 审批；ADR 0009 的架构审批也不能代替该数据库签核。在批准前，0010 的实现和测试只能作为评审候选，不得宣称 release-ready。
