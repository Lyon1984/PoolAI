...
11. `0011_supply_control_plane_m2_e2.sql`：M2-E2 候选以前向约束落实 ADR 0010 的精确 Account Base URL 语法，增加 Account/Channel/Group Supply Configuration 窄写入口与脱敏 readiness observer，收回 API 对四张 Supply 表的直接写权限，且不改写 0001–0010；仍待独立数据库签核。
...
1. 预置三个运行时角色后，空库按 0001/0002/0003/0004/0005/0006/0007/0008/0009/0010 执行；验证缺角色、runtime owner 可登录、API/Worker 继承 runtime owner、禁止的角色属性、重复 migration 与 checksum 漂移都被拒绝。签核不构成远程执行授权，任何远程 migration 仍需另行明确批准。
