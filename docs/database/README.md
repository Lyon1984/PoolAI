...
0015 是待独立数据库签核的 M3-E4 候选，当前 SQL SHA-256 为 `d14a9dcf2dea98867c8c973ce850a2e122e4679d990701e36d7affb2d0181516`，release manifest 为 `15..15`、PostgreSQL required major 为 `18`。该候选只建立 dead Outbox 安全重放所需的窄函数与 NOLOGIN owner 精确列权限，不授予 API、Worker 或 PUBLIC 新的直接表访问；在 `@Lyon1984` 发布精确 APPROVED 评论前不得视为已签署或冻结。候选、测试与后续签核均不授权远程 migration、数据修复、部署、M3 Exit、发布或生产验收。
