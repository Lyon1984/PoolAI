...
0013 当前是 M3-E1 独立数据库签核候选，SQL SHA-256 为 `30abc7bb2858f022247a721503bd58135b715670aec13ca670a2a33cccfe4c44`，release manifest 兼容窗口为 `13..13`，PostgreSQL required major 为 `18`，且冻结 OpenAPI SHA-256 仍为 `ba965851bd6b9b4996ecab2bdf9c947e77981a40e559005ff6618a5269015afe`。候选只新增两个 API-only quota period wrapper，不修改既有表、constraint、Trigger 或 0006 已加入 `auditor` 的 `audit_logs` actor 约束。只有绑定精确候选 commit、SQL checksum、manifest 与权限审计摘要的 Issue #44 永久 APPROVED 评论才能完成数据库签核；候选、测试和后续签核均不授权远程 migration、数据修复、部署、M3 Exit、发布或生产验收。
...
1. 预置三个运行时角色后，空库按 0001/0002/0003/0004/0005/0006/0007/0008/0009/0010/0011/0012/0013 执行；验证缺角色、runtime owner 可登录、API/Worker 继承 runtime owner、禁止的角色属性、重复 migration 与 checksum 漂移都被拒绝。签核不构成远程执行授权，任何远程 migration 仍需另行明确批准。
