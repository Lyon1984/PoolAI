...
12. `0012_supply_account_health_m2_e4.sql`：M2-E4 前向候选增加 Account health observation 一致性约束与唯一的 Supply-owned health transition 入口，撤销 Worker 对 health/cooldown/version 的直接列写权限，以等待行锁后的 observation 时序拒绝迟到或冲突回写，并在 Account 行边界要求规范化 Base URL authority 变化与 credential revision 推进原子发生；不改写 0001–0011。
...
0012 是尚待 `@Lyon1984` 独立数据库签核的 M2-E4 前向候选；当前候选 SQL SHA-256 为 `d41abf363c856172c27812313213c76a54c0ce458d250249adc02ad127ef4dc7`，release manifest 兼容窗口为 `12..12`，PostgreSQL required major 保持 `18`。该登记只把 migration 字节与构建时 checksum 门绑定，不代替 ADR 0011、三个 breaker Lua 或数据库的独立签核，也不授权远程 migration、数据修复、真实 upstream/credential、部署、M2-E4/M2 Exit、发布或生产验收。
...
27. 对候选 0012，以真实 `poolai_api`/`poolai_worker` 验证两者对 Account health/cooldown/version 的直接 UPDATE 均 permission denied，`poolai_supply_record_account_health` 由 NOLOGIN owner 持有、固定 search path 且 EXECUTE allowlist 只有 owner/API/Worker。覆盖非法 health/retry 时间组合、未知与 retired Account、duplicate、旧 observation、同时间冲突以及双连接等待行锁后的迟到判定；applied 只推进公开 version/updated_at，保持 lifecycle、credential revision/envelope 不变，并与调用方当前 UoW 的 audit 原子提交。数据库 sign-off 仍须与 ADR 0011 和三个 breaker Lua 的独立批准分开，且不授权远程 migration、真实 upstream/credential 操作、部署、M2-E4/M2 Exit 或发布。
