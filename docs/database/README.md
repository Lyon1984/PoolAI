...
0014 当前是 M3-E1 独立数据库签核候选，SQL SHA-256 为 `931a35ed64ae6e711ddbc4cf9ca18503ac62f46da6688c6a0efcd91592eb3fea`，release manifest 兼容窗口为 `14..14`，PostgreSQL required major 为 `18`，且 OpenAPI SHA-256 保持 `ba965851bd6b9b4996ecab2bdf9c947e77981a40e559005ff6618a5269015afe`。候选只新增 forward-only representation epoch、一个 quota-owned Trigger/function，并以相同签名前向替换 0013 wrapper；0002、0013 与 OpenAPI 原字节均不改变。只有绑定精确候选 commit、SQL checksum、manifest、两条 DO 审计摘要与真实 PostgreSQL 18 升级验证的 Issue #44 永久 APPROVED 评论才能完成数据库签核；候选、测试和后续签核均不授权远程 migration、数据修复、部署、M3 Exit、发布或生产验收。
