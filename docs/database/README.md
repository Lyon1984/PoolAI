...
9. `0009_identity_api_key_text_validation_m1_e5.sql`：M1-E5 未签署前向纠正候选，以冻结的 Unicode scalar/White_Space/control 规则验证 API Key name 与持久 revoke/rotate reason，替换四个既有函数的同签名定义与两条 length-only constraint；不改写已签 0008。
...
0009 目前只是由 [Proposed ADR 0008](../architecture/adr/0008-freeze-api-key-unicode-scalar-text-contract.md) 治理的本地未签署候选。其 SQL SHA-256 是 `d09cb5e7f1054e7dc1870253fd0da5bf4a1eca6b94328c41cdae544e98bb2270`，release manifest 候选窗口为 `9..9`，对应 OpenAPI SHA-256 是 `1c9dee2fe48cd3e2f0fa5a00805e07e21d303b5a4fa070faeab66f3be6132141`。本地 PostgreSQL 18 Testcontainers 已验证空库 0001..0009、函数/constraint/ACL、补充平面 scalar、控制字符拒绝及合法首尾 White_Space 原样保存；这些证据不构成 `@Lyon1984` 签核。ADR/window 仍为 `Proposed`/`proposed`、approval evidence 为 null，兼容命令必须以 `pending approval` 失败；不得据此执行远程 migration、部署或发布。
...
- API Key 必须且只能属于一个 Group。数据库 Trigger 禁止改 `group_id`；换组必须 revoke 原 Key 后新建。Key hash 为带版本 pepper 的 32 字节 HMAC-SHA-256，只展示 prefix。Proposed ADR 0008/forward 0009 候选进一步规定 name 按 Unicode scalar `1..100`、reason 按 `1..500`，必须含至少一个不属于冻结 25-scalar White_Space 集合的值，拒绝 C0/C1、U+2028/U+2029 与无效 scalar，并原样保留合法首尾 White_Space；在明确批准前它仍是候选而非现行已签边界。
...
1. 预置三个运行时角色后，空库按 0001/0002/0003/0004/0005/0006/0007/0008 执行；对 Proposed 0009 候选的本地验证再追加 0009，但在明确签核前不得远程执行。验证缺角色、runtime owner 可登录、重复 migration 与 checksum 漂移都被拒绝。
