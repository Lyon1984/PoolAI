...
API Key lifecycle 与凭据格式由 [ADR 0007](../architecture/adr/0007-freeze-api-key-lifecycle-and-validation-contract.md) 冻结。精确幂等重放最先返回；首次 update/rotate 再通过 Identity Snapshot 确定 owner/不可变 Group/lifecycle/version，terminal revoked 必须在 Subscription 读取前返回 `409 api_key_revoked`。create 要求 null/未来 expiry 与有效 Subscription；enable、从 expired 恢复 effective active 以及 rotate 都重新验证该 Subscription，缺失/失效分别返回 `subscription_required`/`subscription_inactive`；Admin 代理操作以目标 Key owner 的 Subscription 执行同一判定。未 revoked 但已过期的 rotate 或其他非终态非法转换返回 `409 resource_conflict`。Identity owner command 锁后复核 Snapshot；rotate 继承旧 Key 的 name/Group/expiry/CIDR，在一个 Identity UoW 内创建 active 新 Key并撤销旧 Key，不得原行换 hash。`group_id` 在所有 update/rotate request 中物理缺席。

完整 secret 是发布后不可变的配置 prefix 加 32-byte CSPRNG 无填充 base64url，digest 是专用 versioned pepper 对 `UTF8("PoolAI:ApiKey:v1:" + presented_secret)` 的 HMAC-SHA256。普通 GET/list、Audit、Outbox、错误和日志只见非秘密 display prefix；同一逻辑 create/rotate 的幂等重放只能从 AEAD envelope 恢复完整响应。其 `entity_type=idempotency-request-binding`，`entity_id` 按 `actor_user_id/scope/idempotency_key/request_hash/response_resource_id/response_field` 的固定 canonical 顺序派生；create/rotate 的 `response_field` 与 `field_name` 分别固定为 `api_key_create_response`/`api_key_rotate_response`，解密后 body ID、Location、ETag 必须与绑定的新 Key ID/version 一致。

create 的 `allowed_cidrs` 省略/空数组表示不按来源地址限制；merge PATCH 省略表示不变，显式空数组才清除限制。非空值必须是 IPv4/IPv6 CIDR，拒绝 zone identifier、IPv4-mapped IPv6、歧义前导零 IPv4 与无效地址/前缀，清零 host bits 后规范化、去重并按 ordinal 排序。响应始终返回 canonical 数组。M1-E5 不提前声明 Gateway 已执行来源地址准入；后续数据面只可使用受信代理策略解析出的 canonical client IP。
...
- User、Group、Group Supply Configuration、Subscription Template、Subscription、API Key、Account、Channel 以及 Group quota
  ...
  API Key 的 self/Admin create、GET、PATCH、DELETE、rotate 成功响应以及 `412` 以专用 OpenAPI
  Header/Response component 将强 ETag 标为必返，不能只依赖文字约定。
- 高风险变更必须记录非空理由：User role/status、Group status、Group Supply Configuration 的 channel/account bindings、Account
  credential/status、Channel/Template status、Subscription 分配/更新、本人改密，以及 Admin 代用户创建/更新 Key、本人/Admin
  轮换 Key 或 Admin 发起密码重置，在 JSON body 使用 `reason`；DELETE 撤销/退役（含本人/Admin 撤销 Key）使用
...
secret 只在创建/轮换的首次成功响应及同一绑定幂等请求的安全重放中返回；Account credential
secret 只允许出现在创建/替换输入中。两者都不得通过后续读接口、错误、审计差异或日志回显。
