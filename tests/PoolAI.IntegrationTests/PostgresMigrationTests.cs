...
public sealed partial class PostgresMigrationTests
{
    ...
    public async Task PostgreSql18MigrationAndRuntimeInvariantsHold()
    {
        ...
        await AssertIdentityEntryPointSecurityAsync(connectionString, cancellationToken)
            .ConfigureAwait(true);
        await AssertM1E4RuntimePermissionsAsync(connectionString, cancellationToken)
            .ConfigureAwait(true);
        await AssertM1E4LifecycleAndClockAsync(connectionString, cancellationToken)
            .ConfigureAwait(true);
        ...
        await AssertM3E1ArchiveConcurrencyAsync(connectionString, cancellationToken)
            .ConfigureAwait(true);
        await AssertM3E4OutboxReplayPermissionsAsync(connectionString, cancellationToken)
            .ConfigureAwait(true);
        ...
        await AssertM1E5LifecycleAndClockAsync(connectionString, cancellationToken)
            .ConfigureAwait(true);
        await AssertM1E5TextValidationAsync(connectionString, cancellationToken)
            .ConfigureAwait(true);
    ...
    public async Task ComposeEquivalentMigratorCanApplyRuntimePermissionMigration()
    {
        ...
        await AssertM3E1RuntimePermissionsAsync(
            administratorConnectionString,
            cancellationToken).ConfigureAwait(true);
        await AssertM3E4OutboxReplayPermissionsAsync(
            administratorConnectionString,
            cancellationToken).ConfigureAwait(true);
        ...
        await AssertM1E5RuntimePermissionsAsync(
            administratorConnectionString,
            cancellationToken).ConfigureAwait(true);
        await AssertM1E5TextValidationPermissionsAsync(
            administratorConnectionString,
            cancellationToken).ConfigureAwait(true);
        await AssertM2E1AccountCredentialPersistenceAsync(
            administratorConnectionString,
            cancellationToken).ConfigureAwait(true);
