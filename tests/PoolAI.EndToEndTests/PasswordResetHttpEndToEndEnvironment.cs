...
internal sealed class PasswordResetHttpEndToEndEnvironment : IAsyncDisposable
{
    ...
    internal IHost CreateSupplyHealthHost()
    {
        Dictionary<string, string?> configurationValues = new(
            apiConfiguration,
            StringComparer.OrdinalIgnoreCase)
        {
            ["Data:Postgres:ConnectionString"] = workerConnectionString,
        };
        ...
        builder.Services
            .AddPoolAiPostgresRuntime(workerConnectionString)
            .AddSupplyModule(builder.Configuration, builder.Environment.EnvironmentName)
    ...
    internal static ValueTask RunSupplyHealthRoundAsync(
        IHost healthHost,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(healthHost);
        AccountHealthWorkerService worker = healthHost.Services
            .GetServices<IHostedService>()
            .OfType<AccountHealthWorkerService>()
            .Single();
        return worker.RunSingleRoundAsync(cancellationToken);
    }
